...
uint8_t gc_execute_line(char *line)
{
      ...
      case 'M':
        ...
        switch(int_value) {
          ...
          case 106: case 107:
            ...
            switch(int_value) {
              case 106: gc_block.modal.fan |= FAN_ENABLE; break;
              case 107: gc_block.modal.fan = FAN_DISABLE; break;
            }
            break;
  ...
  */
  ...
  if (gc_parser_flags & GC_PARSER_JOG_MOTION) {
    ...
    // Initialize planner data to current spindle, coolant and fan modal state.
    pl_data->spindle_speed = gc_state.spindle_speed;
    plan_data.condition = (gc_state.modal.spindle | gc_state.modal.coolant | gc_state.modal.fan);
  ...
  // [10. Coolant control ]:
  if (gc_state.modal.fan != gc_block.modal.fan) {
    // NOTE: Fan M-codes are modal. Only one command per line is allowed. But, fan disable clear state.
    fan_sync(gc_block.modal.fan);
    gc_state.modal.fan = gc_block.modal.fan;
  }
  pl_data->condition |= gc_state.modal.fan; // Set condition flag for planner use.
  ...
  if (gc_state.modal.program_flow) {
    ...
    if (gc_state.modal.program_flow == PROGRAM_FLOW_PAUSED) {
      ...
      gc_state.modal.coolant = COOLANT_DISABLE;
        ...
        #endif
      #endif
      gc_state.modal.fan = FAN_DISABLE;
      ...
      // Execute coordinate change and spindle/coolant/fan stop.
      if (sys.state != STATE_CHECK_MODE) {
        if (!(settings_read_coord_data(gc_state.modal.coord_select,gc_state.coord_system))) { FAIL(STATUS_SETTING_READ_FAIL); }
        system_flag_wco_change(); // Set to refresh immediately just in case something altered.
        spindle_set_state(SPINDLE_DISABLE,0.0);
        coolant_set_state(COOLANT_DISABLE);
        fan_set_state(FAN_DISABLE);
      }
